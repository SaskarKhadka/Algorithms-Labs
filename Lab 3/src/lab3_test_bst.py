...
class BSTTestCase(unittest.TestCase):
    ...
    def test_tree_size(self):
        """
        tests fopr size of tree
        """
        bst_tree = BinarySearchTree()

        self.assertEqual(bst_tree.size(), 0)
        self.assertEqual(self.bst.size(), 8)

        bst_tree.add(88, "Value for 88")
        self.assertEqual(bst_tree.size(), 1)

        bst_tree.add(100, "Value for 88")
        self.assertEqual(bst_tree.size(), 2)

        bst_tree.add(101, "Value for 88")
        self.assertEqual(bst_tree.size(), 3)
    ...
    def test_empty(self):
        ...
        bst_tree = BinarySearchTree()
        self.assertEqual(bst_tree.is_empty(), True)

        bst_tree.add(10, "Value for 10")
        self.assertEqual(bst_tree.is_empty(), False)

    def test_invalid_cases(self):
        """
        tests for invalid call to bst functions
        """
        # Delete invalid key
        self.assertEqual(self.bst.size(), 8)
        self.assertEqual(self.bst.remove(200), False)
        self.assertEqual(self.bst.size(), 8)

        # Add duplicate key
        self.assertEqual(self.bst.add(40, "Value for 40"), False)

        # Search invalid key
        self.assertEqual(self.bst.search(200), False)

        bst_tree = BinarySearchTree()

        # Largest, smallest and remove in empty tree
        self.assertEqual(bst_tree.largest(), False)
        self.assertEqual(bst_tree.smallest(), False)
        self.assertEqual(bst_tree.remove(10), False)

        # Traversals in empty tree
        self.assertListEqual(bst_tree.inorder_walk(), [])
        self.assertListEqual(bst_tree.preorder_walk(), [])
        self.assertListEqual(bst_tree.postorder_walk(), [])
